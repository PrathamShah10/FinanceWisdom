...
import { IChats } from "../../interface/user";

const Chat = () => {
  const dispatch = useAppDispatch();
  const { user, chats } = useAppSelector((state) => state.user);
  const [allMessages, setAllMessages] = useState<Array<IChats>>(chats || []);
  useEffect(() => {
    return () => {
  ...
  useEffect(() => {
    user?._id && dispatch(getAllChats(user?._id));
    chats && setAllMessages(chats);
  ...
  const handleMessages = useCallback(
    ...
    [receiverId, senderId, allMessages]
  );
              ...
              >
                ...
                >
                  {chat.sender !== senderId && (
                    <img
                      src={
                        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQjbDPh8tgMF2BNDoBgKgON-3IZqqOWYpRH1g&usqp=CAU"
                      }
                      alt={""}
